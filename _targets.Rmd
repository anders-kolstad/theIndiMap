...
##| tar_interactive: FALSE
compile_profiles <- function(list) {
  purrr::map_df(list, function(x)
                             read.csv(x,
                                header=T) %>%
...
```{targets pp-import}
##| tar_interactive: FALSE
tar_target(import_publications, compile_profiles(ppls),
...
```{targets ip-import}
#| tar_interactive: FALSE
tar_target(import_indicators, compile_profiles(ipls),
           packages = c("tidyverse", "dplyr", "purrr", "readr", "tidyr"))

```

## Resample and create dummy data
Let's take ìmport_indicators
...
DT::datatable(tar_read(import_publications),
              options = list(
                pageLength = 5))
```

```{r indicators-dt, echo=F}
DT::datatable(tar_read(import_indicators),
              options = list(
                pageLength = 5))
```
