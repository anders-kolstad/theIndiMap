...
```{r, eval = FALSE}
install.packages(c("tidyverse", "reactable", "tmap", "countries"))
```
...
tar_option_set(packages = c("tidyverse", "dplyr", "purrr", "readr", "tidyr", "ggplot2", "tmap", "countries")) # I had to mention dplyr, purrr ect even though they are included in tidyverse.
...
```{targets scramble_mc, tar_globals=T}
scramble_mc <- function(x) {
  vector <- x |> 
    as.data.frame() |>
...
scramble <- function(data) {
  ...
  data[sample(1:nrow(data), 150, replace = T),] |>
    ...
    mutate(across(any_of(mc),scramble_mc))
}
...
## Map
```{targets myMap, tar_globals = TRUE}

myMap <- function(data){
  
# Melt data frame and convert to three letter country code to match the World data in {tmap}.
# Then count the number of indicators per country
dat2 <- data |>
  separate_longer_delim(iCountry, " | ") |>
  filter(iCountry != "") |>
  group_by(iCountry)|>
  summarise(n = n()) |>
  mutate(ISO3 = countries::country_name(iCountry, to = "ISO3", verbose=T))

# Get the World data fro tmap
data("World")

# Join the two data sets
World <- World %>%
  left_join(dat2, by=join_by(iso_a3 == ISO3))

# Plot world map, with colour and superimposed numbers for the countries with indicators
tm_shape(World, projection = 8857) + # equal area projection
  tm_polygons()+
  tm_shape(World[!is.na(World$n),])+
...
create_bar <- function(data, x) {
  data |>
    ...
    ggplot() +
    geom_bar(aes_string(x = x, y = "indicators"),
             fill="grey",
             colour="grey30",
             stat = "identity") +
    theme_minimal(18)+
...
## Map
```{targets worldMap_dummy}
tar_target(worldMap_dummy, myMap(indicators_dummyData), packages = c("tidyverse", "tmap", "countries"))
```
