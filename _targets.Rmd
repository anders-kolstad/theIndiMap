...
Here is a function to take a vector (data frame column) og length x, make it longer based on a deliminator to become x+z, sample to length of x, and sample some again to combine and make multiple choice fields. Later we can use this function inside mutate(across()).

```{r}
distinct_ms <- function(x) {
  vector <- x |> 
    as.data.frame() |>
  ...
  secondvalue <- temp[secondvalue!= ""] # rm blanks
